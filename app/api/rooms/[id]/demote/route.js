...
export async function POST(request, { params }) {
  ...
  try {
    const participants = await demoteToListener(id, identity.anonymousId, body.handle);
    return NextResponse.json({ participants });
  } catch (e) {
    const status = e.code === "NOT_HOST" ? 403 : e.code === "NOT_FOUND" ? 404 : e.code === "CANNOT_DEMOTE_HOST" ? 400 : 500;
