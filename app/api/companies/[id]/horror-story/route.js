...
import { addCompanyHorrorStory } from "@/lib/content/service";
...
export async function POST(request, { params }) {
  ...
  if (!story) return NextResponse.json({ error: "Story text is required." }, { status: 400 });
  ...
  const company = await addCompanyHorrorStory({
    companyId: id, anonymousId: identity.anonymousId, authorDisplay: identity.displayLabel, story,
  });
